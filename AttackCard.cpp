...
AttackCard::AttackCard(CardType t, string n, string d, int e, int damage, int upgradeddamage) :
    // FIX: upgradeddamage parameter was never assigned to the upgradedDamage
    // member, leaving it uninitialized (UB). Any card that got upgraded via
    // AttackCard::upgrade() (which does `baseDamage = upgradedDamage;`) would
    // then deal a garbage amount of damage.
    Card(t, n, d, e), baseDamage(damage), upgradedDamage(upgradeddamage){}
...
void ReaperCard::applyEffect(class Character* caster, class Character* target, BattleManager* bm) {
    if (caster && target) {
        // FIX: this declared a new local `int baseDamage` and initialized it
        // from itself (`baseDamage` on the right-hand side resolved to the
        // still-uninitialized local, not the AttackCard::baseDamage member,
        // because the local shadows the member for the rest of its scope).
        // That's undefined behavior — reads garbage, dealt random damage,
        // and could crash. Use this->baseDamage explicitly, same as FeedCard.
