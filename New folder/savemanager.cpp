...
{
    if (username.isEmpty() || save_name.isEmpty()) {
        qDebug() << "Invalid save parameters: empty username or save_name";
        return false;
    }
    if (score < 0 || floor < 0) {
        qDebug() << "Invalid save parameters: negative score or floor";
        return false;
    }
...
{
    if (save_id <= 0 || score < 0 || floor < 0) {
        qDebug() << "Invalid update save parameters";
        return false;
    }
    ...
    // FIX: return false if save_id didn't exist
...
{
    if (save_id <= 0) {
        qDebug() << "Invalid save_id for delete";
        return false;
    }
...
{
    if (save_id <= 0) return QJsonObject();
