...
bool user_manager::verify_email(const QString &username, const QString &code)
{
    ...
    // FIX: clear verification_code after successful verify so it can't be reused
    updateQuery.prepare("UPDATE users SET is_verified = 1, verification_code = NULL WHERE username = :username");
...
bool user_manager::reset_password(const QString &username, const QString &new_password)
{
    ...
    QSqlQuery query;
    // FIX: clear verification_code after password reset so old reset code becomes invalid
    query.prepare("UPDATE users SET password = :password, verification_code = NULL WHERE username = :username");
    query.bindValue(":password", hashedPassword);
    query.bindValue(":username", username);
