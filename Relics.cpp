...
Relic* createRelicByName(const std::string& name) {
    if (name == "PreservedInsect") return new PreservedInsect();
    if (name == "SlaversCollar") return new SlaversCollar();
    if (name == "CultistHeadpiece") return new CultistHeadpiece();
    return nullptr;
}